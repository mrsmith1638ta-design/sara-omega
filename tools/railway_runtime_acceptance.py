...
The script never prints the owner token. Restore is intentionally opt-in because it
mutates live runtime state.
"""
...
def main() -> int:
    ...
    def check(method: str, path: str, *, headers=None, expected=(200,)):
        nonlocal failed
        response = client.request(method, base_url + path, headers=headers)
