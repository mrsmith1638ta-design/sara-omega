...
For the AWS health surface, the release gate reads non-secret evidence fields from environment configuration:

- `SARA_AWS_ROAD_ACCEPTANCE=PASS`
- `SARA_AWS_PRODUCTION_ACCEPTED=true`
- `SARA_AWS_ACCEPTANCE_SOURCE=<live acceptance evidence source>`
- `SARA_AWS_ARTIFACT_SIGNING=PASS`
- `SARA_AWS_ARTIFACT_SHA256=<64-character artifact sha256>`
- `SARA_AWS_SIGNATURE_REF=<signing evidence reference>`
- `SARA_AWS_PROMOTION_AUTHORITY=APPROVED`
- `SARA_AWS_PROMOTION_APPROVER=<authorized approver>`
- `SARA_AWS_PROMOTION_SCOPE=<approved release scope>`

If any required field is missing, malformed, or only a placeholder, the release gate remains blocked.
