...
def rho(n: int, x: float) -> float:
    ...
    if not 0.0 < x < 1.0:
        raise ValueError("x_must_lie_in_open_unit_interval")
    value = 1.0 / (n * x)
    return value - math.floor(value)


def gram_entry(j: int, k: int, samples: int | None = None) -> float:
...
def gram_matrix(n: int, samples: int | None = None) -> list[list[float]]:
    if n < 1:
        raise ValueError("n_must_be_positive")
...
def mobius_residual(N: int, samples: int | None = None) -> RiemannResult:
    if N < 1:
        raise ValueError("N_must_be_positive")
    data = residual_against_negative_mobius([0, *mobius_values(N)])
    ...
    return RiemannResult(
        ...
        evidence=["exact finite Vasyunin Gram computation"],
